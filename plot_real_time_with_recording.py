...
class Graph:
    def __init__(self, board_shim):
        ...
        self.battery_voltage_channel = BoardShim.get_battery_channel(self.board_id,
    ...
    def _init_controls(self):
        ...
        self.stop_button = QtGui.QPushButton('FORCE STOP RECORDING')
        self.duration_input = QtGui.QLineEdit()
        self.duration_input.setPlaceholderText('Duration (seconds). Default - 24h')

        self.file_note_input = QtGui.QLineEdit()
        self.file_note_input.setPlaceholderText('Filename note')

        # Add buttons and input field to layout
        layout = QtGui.QVBoxLayout()
        widget = QtGui.QWidget()
        layout.addWidget(self.duration_input)
        layout.addWidget(self.file_note_input)

        layout.addWidget(self.record_button)

        layout.addWidget(self.stop_button)
        widget.setLayout(layout)
        # self.win.addItem(widget, row=11, col=0, colspan=2)

        proxy = QtGui.QGraphicsProxyWidget()
        proxy.setWidget(widget)

        self.win.addItem(proxy, row=11, col=0, colspan=2)
    ...
    def start_recording(self):
        ...
        try:
            ...
            logging.warning("Invalid duration input. Using default - 24h")
            duration = 60 * 60 * 24

        now = datetime.datetime.now()
        self.filename = now.strftime(
            f"experimental_data/streamer_{now:%Y_%m_%d_%H_%M_%S}_{self.file_note_input.text()}.csv"
        )
        logging.info(f"Started recording to {self.filename}")
        self.board_shim.add_streamer(f"file://{self.filename}:w")
        self.recording = True
    ...
    def stop_recording(self):
        if self.recording and self.filename:
            self.board_shim.delete_streamer(f"file://{self.filename}:w")
            self.recording = False
            logging.info(f"Recording stopped and saved to {self.filename}")
...
def main():
    ...
    try:
        ...
        for i in range(8):
            board_shim.config_board(f'{{"command": 3, "channel": {i}, "pga": 128}}\n')
